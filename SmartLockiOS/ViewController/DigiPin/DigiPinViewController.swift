...
struct DigiPin : Hashable,Equatable {
    var name : String = ""
    var pin : String = ""
...
//        var uniquePins = Set<DigiPin>()
//        arrDigiPins = arrDigiPins.filter { uniquePins.insert($0).inserted }
//    }
...
//        LoaderView.sharedInstance.showShadowView(title: "Loading...", selfObject: self, isFromNotifiation: false)
        if  let authorizationKey = UserController.sharedController.authorizationKeyForWifi(isSecured: self.lockListDetailsObj.is_secured){
            ...
            LockWifiManager.shared.addDigiPin(userDetails: parameters, completion: {[weak self] (isSuccess, jsonResponse, error) in
