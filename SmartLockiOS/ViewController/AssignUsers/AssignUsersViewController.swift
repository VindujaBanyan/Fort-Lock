    ...
    func revokeUserViaWIFI(slotNumber: String, userObj: AssignUserModel) {
        if  let authorizationKey = UserController.sharedController.authorizationKeyForWifi(isSecured: self.lockListDetailsObj.is_secured){
           ...
           parameters["slot-id"] = actualSlotNumber
            print("parameters for revoke user \(parameters)")
            //parameters["slot-id"]  = String.stringToHexString(regularString: actualSlotNumber)
            LockWifiManager.shared.rewriteUserSlotKey(userDetails: parameters, completion: {[weak self] (isSuccess, jsonResponse, error) in
...
//                self.present(alert, animated: true, completion: nil)
    ...
    func createUserRequestServiceCall(selectedMobile: String,countryCode: String) {
        ...
        */
        ...
        print("the details for assign user is \(userDetailsDict)")
...
//                headerTitle = "MASTER USER(S)"
            ...
            case 1, 2, 3:
                ...
                if userObj.status == "1" {
                    ...
                    if self.lockListDetailsObj.lockVersion == lockVersions.version2_0.rawValue || self.lockListDetailsObj.lockVersion == lockVersions.version2_1.rawValue || self.lockListDetailsObj.lockVersion == lockVersions.version3_0.rawValue || self.lockListDetailsObj.lockVersion == lockVersions.version3_1.rawValue || self.lockListDetailsObj.lockVersion == lockVersions.version4_0.rawValue || self.lockListDetailsObj.lockVersion == lockVersions.version6_0.rawValue{
                        cell?.fpPrivilegeButton.addTarget(self, action: #selector(onTapFPAccessButton), for: .touchUpInside)
