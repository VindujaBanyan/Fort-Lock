...
//        LockWifiManager.shared.localCache.updateOfflineItems()
    ...
    func uiUpdate(){
        self.rewriteandGenerateOTPUI(isEnableGenerate: true)
    }
    func rewriteandGenerateOTPUI(isEnableGenerate : Bool){
        if self.lockListDetailsObj.lockVersion == lockVersions.version6_0.rawValue {
            self.viewGenerateOtp.isHidden = false // Show Generate OTP button
            self.viewRewriteOtp.isHidden = true // Hide Rewrite OTP button
            self.lblEmptyOTP.text = "Please Generate OTP to the lock to proceed"
        } else {
            self.viewGenerateOtp.isHidden = true // Hide Generate OTP button
            self.viewRewriteOtp.isHidden = false // Hide Rewrite OTP button
            self.lblEmptyOTP.text = "Please Rewrite OTP to the lock to proceed"
        }
...
//        LoaderView.sharedInstance.showShadowView(title: "Loading...", selfObject: self, isFromNotifiation: false)
        if  let authorizationKey = UserController.sharedController.authorizationKeyForWifi(isSecured: self.lockListDetailsObj.is_secured){
            ...
            LockWifiManager.shared.addOTP(userDetails: parameters, completion: {[weak self] (isSuccess, jsonResponse, error) in
