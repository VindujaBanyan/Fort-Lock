...
class CoreDataController: NSObject {
    ...
    //var passageModeSwitch: UISwitch!
    
    func saveLock(lockobject:LockListModel,passageModeSwitch : UISwitch) -> Bool {
      ...
      //Encode values
        ...
        do{
            ...
            if results != nil && results?.count != 0 {
                ...
                managedObject?.setValue(lockobject.enable_passage, forKey: "enable_passage")
                try context?.save()
                print("Updated")
                ...
                do{
                    ...
                    let lock = NSEntityDescription.insertNewObject(forEntityName: entity, into: context!)
                      ...
                      lock.setValue(lockobject.enable_passage, forKey: "enable_passage")
                    try context?.save()
                    print("Saved ")
                }
                catch{
    ...
    func appendToArray(fetchResult:[Any])->[LockListModel]{
        ...
        for i in fetchResult as! [NSManagedObject]{
          ...
          //assign fetched values to object value
            ...
            lockListObj.enable_passage = (i.value(forKey: "enable_passage") as? String)
    ...
    func updateLockPassageMode(for lockID: String, enablePassage: Bool) {
        let appDelegate = UIApplication.shared.delegate as? AppDelegate
        let context = appDelegate?.persistentContainer.viewContext
        
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entity)
        fetchRequest.predicate = NSPredicate(format: "id = %@", lockID)
        
        do {
            let results = try context?.fetch(fetchRequest) as? [NSManagedObject]
            if let lockObject = results?.first {
                // Convert the boolean value to string "1" for true and "0" for false
                lockObject.setValue(enablePassage ? "1" : "0", forKey: "enable_passage")
                
                do {
                    try context?.save()
                    print("Updated enable_passage for lock with ID: \(lockID)")
                } catch {
                    print("Error saving context: \(error.localizedDescription)")
                }
            } else {
                print("Lock with ID \(lockID) not found.")
            }
        } catch {
            print("Error fetching lock: \(error.localizedDescription)")
        }
    }
