...
//    }
    ...
    func utcToLocal(utcDate: String, toFormat: String) -> String {
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            dateFormatter.timeZone = TimeZone(abbreviation: "UTC")
    ...
    func localToTimeZone(date: String, timeZoneAbbreviation: String) -> String {
            let dateFormatter = DateFormatter()
            // Input format to parse the local date-time string
            dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            dateFormatter.calendar = Calendar.current
            dateFormatter.timeZone = TimeZone.current
            ...
            // Change time zone to the specified time zone and set output format
            dateFormatter.timeZone = TimeZone(abbreviation: timeZoneAbbreviation)
            dateFormatter.dateFormat = "dd-MM-yy HH:mm:ss"

            // Convert the local date-time to the specified time zone date-time string
            return dateFormatter.string(from: localDate)
        }
//
...
//    }
    ...
    func toTimeUTC(withFormat format: String = "HH:mm:ss", dateString: String) -> Date {
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = format
            dateFormatter.timeZone = TimeZone(secondsFromGMT: 0) // Interpret the input as UTC
            guard let date = dateFormatter.date(from: dateString) else {
        ...
        // Convert a Date object to a time string, formatting the string as UTC
        func toTimeStringUTC(withFormat format: String = "HH:mm:ss", date: Date) -> String {
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = format
            dateFormatter.timeZone = TimeZone(secondsFromGMT: 0) // Format the output as UTC
            let dateString = dateFormatter.string(from: date)
            return dateString
        }
    ...
    func toTime(withFormat format: String = "HH:mm:ss", dateString: String) -> Date {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format
        dateFormatter.timeZone = TimeZone.current // Ensure it uses the local time zone
        guard let date = dateFormatter.date(from: dateString) else {
            preconditionFailure("Take a look to your format")
        }
        return date
...
extension Utilities {
    func toDateT(dateString: String) -> Date? {
        let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "yyyy-MM-dd"
            return dateFormatter.date(from: dateString)
        }
        
        func toTimeT(timeString: String) -> Date? {
            let timeFormatter = DateFormatter()
                    timeFormatter.dateFormat = "HH:mm:ss"
            return timeFormatter.date(from: timeString)
        }
        
        func toDateTimeT(dateTimeString: String) -> Date? {
           let  dateTimeFormatter = DateFormatter()
                    dateTimeFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            return dateTimeFormatter.date(from: dateTimeString)
        }
        
        func toDateStringT(date: Date) -> String {
            let dateFormatter = DateFormatter()
                    dateFormatter.dateFormat = "yyyy-MM-dd"
            return dateFormatter.string(from: date)
        }
        
        func toTimeStringT(time: Date) -> String {
            let timeFormatter = DateFormatter()
                    timeFormatter.dateFormat = "HH:mm:ss"
            return timeFormatter.string(from: time)
        }
        
        func toDateTimeStringT(dateTime: Date) -> String {
            let  dateTimeFormatter = DateFormatter()
                     dateTimeFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            return dateTimeFormatter.string(from: dateTime)
        }
        
        func currentDateTimeStringT() -> String {
            let  dateTimeFormatter = DateFormatter()
                     dateTimeFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
