...
var CONNECTIVITY_TIME = 13.0
var BLE_ACTIVE_TIME = 7.0
let WIFI_DELAY_TIME: TimeInterval = 3.0
...
let TURN_ON_LOCK_FOR_OTHERS = "Please switch on the lock and try again"
let TURN_ON_LOCK_FOR_V6 = "Long Press 1 on the Lock and Click OK"
...
let ADD_FP_INSTRUCTION = "Long press 1 on the Lock and press below Button"
...
enum lockVersions : String {
    ...
    case version3_2 = "v3.2"
    case version6_0 = "v6.0"
}
...
extension UIViewController {
    ...
    func getActivityTime(lockVersion : lockVersions) -> Double{
        ...
        if let configurationValue = getJSON(UserdefaultsKeys.versionConfiguration.rawValue)
        {
            ...
            case .version3_2:
                strWifiTime = "\(String(describing: configurationValue["v3.2"]["wifi-time"]))"
                BLE_ACTIVE_TIME = Double("\(String(describing: configurationValue["v3.2"]["ble-time"]))") ?? 7.0
            case .version6_0:
                strWifiTime = "\(String(describing: configurationValue["v6.0"]["wifi-time"]))"
                BLE_ACTIVE_TIME = Double("\(String(describing: configurationValue["v6.0"]["ble-time"]))") ?? 7.0
            }
            return Double(strWifiTime) ?? 15.0
        }
        return 13.0
    }
...
//    }
    func getJSON(_ key: String) -> JSON? {
        guard let jsonString = UserDefaults.standard.string(forKey: key), !jsonString.isEmpty else {
            return nil
        }
        ...
        do {
            ...
            // Handle parsing error, you might want to print/log the error
