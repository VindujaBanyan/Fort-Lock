...
class LockWifiLocalCache {
    ...
    func checkAndUpdateOwnerId(completion: @escaping (Bool) -> Void) {
        print("checkAndUpdateOwnerId function works here")
        ...
        if UserDefaults.standard.bool(forKey: "requires_owner_update") == true {
           // LoaderView.sharedInstance.showShadowView(title: "Loading...", isFromNotifiation: false)
            ...
            for lockSerialNumber in lockSerialNumberArray{
                ...
                AssignUsersViewModel().revokeRequestUserServiceViewModel(url: urlString, userDetails: userDetails, serialNumber: lockSerialNumber, callback: { [weak self](result, error, serialNumber) in
...
extension LockWifiLocalCache{
    ...
    func ownerIdsToBeUpdated() -> [String] {
        ...
        print("tobeUpdatedOwnerIdArray ==> \(String(describing: tobeUpdatedOwnerIdArray))")
    ...
    func locksToBeUpdatedForFactoryReset() -> [String] {
        ...
        print("factory reset updates = \(factoryResetUpdates)")
    ...
    func updateOfflineItems(){
        DispatchQueue.global(qos: .background).async {
            ...
            if Connectivity().isConnectedToInternet(){
                ...
                LockWifiManager.shared.localCache.updatePassageMode(completion: { (status) in
...
extension LockWifiLocalCache {
    ...
    func passageModeToBeUpdated() -> [String:AnyObject]? {
        return UserDefaults.standard.value(forKey: "to_be_updated_passage_mode") as? [String:AnyObject]
    }
    
    func setUpdatePassageModeToBeUpdated(switchEnable: [String: AnyObject]?) {
        if switchEnable == nil {
            UserDefaults.standard.removeObject(forKey: "to_be_updated_passage_mode")
        } else {
            UserDefaults.standard.set(true, forKey: "requires_passage_mode_update")
            UserDefaults.standard.set(switchEnable, forKey: "to_be_updated_passage_mode")
        }
    }
    
    func updatePassageMode(completion: @escaping (Bool) -> Void) {
        let requiresUpdate = UserDefaults.standard.bool(forKey: "requires_passage_mode_update")
        
        if requiresUpdate {
            guard let passageModeEnabled = passageModeToBeUpdated(),
                  let lockId = passageModeEnabled["lock_id"] as? String else {
                completion(false) // Return false if passage mode data is missing or invalid
                return
            }
            
            let urlString = ServiceUrl.BASE_URL + "locks/updatelock?id=\(lockId)"
            var userDetails = passageModeEnabled
            userDetails.removeValue(forKey: "lock_id")
            
            DigiPinViewModel().addDigiPinsServiceViewModel(url: urlString, userDetails: userDetails) { (result, error) in
                if result != nil {
                    UserDefaults.standard.removeObject(forKey: "to_be_updated_passage_mode")
                    UserDefaults.standard.set(false, forKey: "requires_passage_mode_update")
                    completion(true)
                } else {
                    // Handle error case
                    completion(false)
...
//    func passageModeToBeUpdated() -> [String:AnyObject]? {
//        let tobeUpdatedUserKeyArray = UserDefaults.standard.value(forKey: "to_be_updated_passage_mode") as? [String:AnyObject]
//        if tobeUpdatedUserKeyArray == nil {
//            return [String:AnyObject]()
//        }
//        return tobeUpdatedUserKeyArray
...
//    func setUpdatePassageModeToBeUpdated(switchEnable: [String: AnyObject]) {
//        if switchEnable == nil {
//            UserDefaults.standard.set([], forKey: "to_be_updated_passage_mode")
//        } else {
//            UserDefaults.standard.set(true, forKey: "requires_passage_mode_update")
//            UserDefaults.standard.set(switchEnable, forKey: "to_be_updated_passage_mode")
//        }
...
//        
//        if requiresUpdate {
//            var passageModeEnabled = passageModeToBeUpdated() ?? [:]
//            let lockId  = passageModeEnabled["lock_id"] as? String ?? ""
//            let urlString = ServiceUrl.BASE_URL + "locks/updatelock?id=\(lockId))"
//            
//            passageModeEnabled.removeValue(forKey: "lock_id")
//            let userDetails = passageModeEnabled
//            DigiPinViewModel().addDigiPinsServiceViewModel(url: urlString, userDetails: userDetails) { (result, error) in
//                if result != nil {
//                    UserDefaults.standard.set([], forKey: "to_be_updated_passage_mode")
//                    UserDefaults.standard.set(false, forKey: "requires_passage_mode_update")
