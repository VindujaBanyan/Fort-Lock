...
extension HomeWifiConfigurationViewController {
    ...
    func updateHomeWiFiConfiguration() {
        ...
        if  let authorizationKey = UserController.sharedController.authorizationKeyForWifi(isSecured: self.lockListDetailsObj.is_secured){
            ...
            let mqttArray = mqttServiceUrl.components(separatedBy: ":")
            if(mqttArray.count == 2){
                parameters["mqtt-ip"] = mqttArray[0]
                parameters["mqtt-port"] = mqttArray[1]
                parameters["mqtt-apikey"] = self.lockConnection.serialNumber
            }
            print("Home WiFi Params : \(parameters)")
            
            LoaderView.sharedInstance.showShadowView(title: "Loading...", selfObject: self, isFromNotifiation: false)
            LockWifiManager.shared.configureHomeWiFiMqtt(wifiDetails: parameters, completion: {[unowned self] (isSuccess, jsonResponse, error) in
                LoaderView.sharedInstance.hideShadowView(selfObject: self)
                if isSuccess == true {
                    if let jsonDict = jsonResponse?.dictionary {
                        let updatedKey = jsonDict["error-message"]?.rawString() ?? ""
                        let tempStr: LockWiFiMqttConfigMessages = LockWiFiMqttConfigMessages(rawValue: updatedKey) ?? .EMPTY
                        switch tempStr {
                        case .OK:
                            print(".OK")
                            let alert = UIAlertController(title: ALERT_TITLE, message: HOME_WIFI_MQTT_OK, preferredStyle: UIAlertController.Style.alert)
