...
public class BluetoothManager : NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    let serialQueue = DispatchQueue(label: "LockQueue")
    ...
    func initCBCentralManager() {
        ...
        _manager = CBCentralManager(delegate: self, queue: serialQueue, options: dic)
    }
    ...
    func startScanPeripheral() {
        _manager?.scanForPeripherals(withServices: nil, options: [CBCentralManagerScanOptionAllowDuplicatesKey:false])
    }
    ...
    func retrievePeripheral(uuidString uuid:String) -> CBPeripheral?{
        let peripherals = _manager?.retrievePeripherals(withIdentifiers: [UUID(uuidString:uuid)!])
    ...
    @objc func connectTimeout(_ timer : Timer) {
        if isConnecting {
            ...
            _manager?.cancelPeripheralConnection(timer.userInfo as! CBPeripheral)
            timeoutMonitor = nil
        }
    }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        delegate?.didWriteValueForForCharacteristic?(characteristic, error)
    }
