...
class FPListViewController: UIViewController {
    ...
    func updateUI() {
        if self.lockListDetailsObj.lockVersion != lockVersions.version3_0.rawValue && self.lockListDetailsObj.lockVersion != lockVersions.version3_1.rawValue && self.lockListDetailsObj.lockVersion != lockVersions.version4_0.rawValue{
...
//                LoaderView.sharedInstance.hideShadowView(selfObject: self)
    ...
    func fpManagePrivilegeViaWIFI() {
        ...
        if  let authorizationKey = UserController.sharedController.authorizationKeyForWifi(isSecured: self.lockListDetailsObj.is_secured){
            ...
            LockWifiManager.shared.updateFPManagePrivilege(userDetails: parameters, completion: {[unowned self] (isSuccess, jsonResponse, error) in
                LoaderView.sharedInstance.hideShadowView(selfObject: self)
                if isSuccess == true {
                    if let jsonDict = jsonResponse?.dictionary {
                        let dictResponse = jsonDict["response"]?.dictionaryObject!
